...
int main() {
    ...
    for (int t = 0; t < 250; ++t) {
        Mat yMat = Mat(cvSize(nWidth, nHeight), CV_8UC3, cv::Scalar(0, 0, 0));
        Mat uMat = Mat(cvSize(nWidth/2, nHeight/2), CV_8UC3, cv::Scalar(0, 0, 0));
        Mat vMat = Mat(cvSize(nWidth/2, nHeight/2), CV_8UC3, cv::Scalar(0, 0, 0));
        ...
        unsigned char* BGR = new unsigned char[FrameSize * 2];
        YV12ToBGR24_Table(pBuf, BGR, nWidth, nHeight);
        int loc = -1;
        for (int x = 0; x < nHeight; ++x)
            for (int y = 0; y < nWidth; ++y) {
                int index = x*nWidth+y;
                rgbPtr[index*3+2] = BGR[++loc];
                rgbPtr[index*3+1] = BGR[++loc];
                rgbPtr[index*3+0] = BGR[++loc];
            }
